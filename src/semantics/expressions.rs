...
impl<'a, 'b, 'c> LightExpressionsGenerator<'a, 'b, 'c> {
    ...
    pub fn calculate(&self, expr: &Expr, expected: Option<&Type>) -> LExprTyped {
        let calculated_type = match expr {
            ...
            Expr::Identifier(i) => {
                let identifier_type = self.variables_types.get(i).expect(&format!("No identifier {} found", i));
            ...
            Expr::This => {
                ...
                let identifier_type = self.variables_types.get("name").expect("This is not in scope of method!");
